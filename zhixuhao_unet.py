...
class myUnet(object):
    ...
    def train(self):
        ...
        x_train, y_train = self.expandDataSet(train_dset, samples=10)
        ...
        model_checkpoint = ModelCheckpoint('unet.hdf5', monitor='loss',verbose=1, save_best_only=True)
       	# only saves the best model depending on optimal loss across epochs.
	 print('Fitting model...')
        
        print('Training Model...')

        model.fit(x_train, y_train, batch_size=1, nb_epoch=10, validation_data= (x_val, y_val), verbose=1, callbacks=[model_checkpoint])
        print('Trained Model')
        '''
        print('Predict Test Data')
        
	# if we are doing a test, test on validation set not training set!
        a_slice = x_train[None, 2, :, :, :]
        import time
        t0 = time.time()
